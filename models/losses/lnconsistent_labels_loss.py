...
def merge_label(label, class_flag):
    merged_label = torch.zeros_like(label).to('cuda')
    cc = 0
    for c, class_exist in enumerate(class_flag):
        if c > 0 and class_exist > 0:
...
def excluse_label(label):
    exclused_label = torch.zeros_like(label).to('cuda')
    exclused_label[label > 0] = 1
    return exclused_label


class mar_loss(nn.Module):
    def __init__(self,):
        super(mar_loss, self).__init__()
        self.dice_loss = DiceLoss()
        self.ce_loss = nn.NLLLoss()
    def forward(self, prob, target, class_flag):
        ...
        l_ce = self.ce_loss(margin_log_prob, margin_target.squeeze(dim=1))
        margin_target_oh = make_onehot(margin_target, cls=np.sum(class_flag))
        l_dice = self.dice_loss(margin_prob, margin_target_oh)
        return l_ce, l_dice

class exc_loss(nn.Module):
    def __init__(self,):
        super(exc_loss, self).__init__()
        self.dice_loss = DiceLoss()
    ...
    def forward(self, prob, target, class_flag):
        ...
        for c, class_exist in enumerate(class_flag):
            if c == 0 or class_exist == 0:
                ...
                l_dice += 1 - self.dice_loss(prob[:, c:c + 1, :], exclused_target)
                l_dice_num += 1
        if l_ce_num > 0:
            l_ce = l_ce / l_ce_num
        if l_dice_num > 0:
...
def dice_and_ce_loss(prob, target, class_flag):
    ...
    for c, class_exist in enumerate(class_flag):
        if c > 0 and class_exist > 0:
            ...
            # bin_prob = torch.cat([prob[:,0,:][:,None,...], prob[:,c:c+1,:]], dim=1)
