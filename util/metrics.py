...
def get_total_evaluation(pred: np.ndarray, mask: np.ndarray, require_edge: bool = True, bg_value=0):
    ...
    if require_edge:
        ...
        # metric_values['dice'] = 2*metric_values['miou']/(1+metric_values['miou'])
        ...
        try:
            ...
            metric_values['rel_grain_size'] = 0
    else:
...
def DICE(Vref, Vseg):
    ...
    d2 = []
    ds = []
    for i in np.unique(Vref):
        if i != 0:
            vr = np.zeros_like(Vref)
            vr[Vref==i] = 1
            vs = np.zeros_like(Vseg)
            vs[Vseg == i] = 1
            d = 2 * (vr & vs).sum() / (vs.sum() + vr.sum() + 1e-5)
            # dice += d
            d1.append(2 *(vr&vs).sum())
            d2.append(vs.sum() + vr.sum())
            ds.append(d)

    # dice = dice/(len(np.unique(Vseg))-1)
    # dd = 0
    ...
    # print(dice)
    # exit()
    # dice = 2*np.array(d1).sum()/np.array(d2).sum()
    return dice
