...
class Client:
    ...
    def train(self, w_global, round_idx):
        ...
        if round_idx>0 and self.args.federated_algorithm == 'fedseg':
            print('Extracting prototypes...')
            proto_tmp, label_list, label_mask_ = self.model.get_protos(model=copy.deepcopy(self.model.net),
                                                                        global_round=round_idx, trainloader_eval=self.local_training_data)
            # if round_idx >= 0:
        ...
        global_model = copy.deepcopy(self.model.net)
        global_model.eval()
        for epoch in range(self.args.epochs):
            ...
            for i, data in tqdm.tqdm(enumerate(self.local_training_data)):
                ...
                if self.args.federated_algorithm == 'fedddpm':
                    ...
                    self.model.feseg_optimize_parameters(round_idx,local_mem,local_mask,global_model)
                else:
